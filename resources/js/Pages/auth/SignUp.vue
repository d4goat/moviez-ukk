...
const { mutate: sendOtp, isLoading: isLoadingSendOtp } = useMutation({
    ...
    onSuccess: () => {
        ...
        ElMessage.success("Your verification code has been sent to your email")
        activeTab.value = 3
    },
    onError: (err: any) => {
...
const { mutate: verifyOtp, isLoading: isLoadingVerifyOtp } = useMutation({
    ...
    onSuccess: () => {
        ...
        ElMessage.success("Success create account")
    },
    onError: (err: any) => {
        ElMessage.error(err.response.data.message)
        console.error(err.response.data.message)
...
<template>
    <VForm :validation-schema="formSchema" ref="formRef" id="form-register"
        ...
        <div
            class="w-full max-w-sm md:max-w-md lg:max-w-lg flex flex-col justify-center bg-border p-8 rounded-xl space-y-8">
            <!-- begin:title -->
            ...
            <div class="flex flex-col gap-6">
                <div v-if="activeTab === 1" class="flex flex-col gap-6">
                    ...
                    <div class="flex flex-col gap-2">
                        <div class="flex flex-col">
                            <Field name="phone" type="text" autocomplete="off" v-model="user.phone"
                                class="w-full bg-[#232323] border-none focus:border-2 focus:border-[#7C7C7C] focus:ring-2 focus:ring-[#7C7C7C] rounded-xl p-3 text-sm sm:text-sm">
                                <InputGroup>
                                    <InputGroupAddon>
                                    ...
                                    <FloatLabel variant="in">
                                        <InputText v-model="user.phone"
                                            oninput="this.value = this.value.replace(/[^\d,]/g, '')" class="w-full"
                            ...
                            <div class="flex flex-col relative">
                                <Field name="password" type="password" autocomplete="off" v-model="user.password"
                                    ...
                                    <InputGroup>
                                        ...
                                        <FloatLabel variant="in">
                                            <Password class="w-full" input-class="w-full" v-model="user.password"
                                                toggle-mask input-id="password" :feedback="false" />
                            ...
                            <div class="flex flex-col relative">
                                <Field name="passwordConfirmation" type="password" autocomplete="off"
                                    ...
                                    <InputGroup>
                                        ...
                                        <FloatLabel variant="in">
                                            <Password v-model="user.passwordConfirmation" class="w-full"
                                                input-class="w-full" toggle-mask input-id="password-confirmation"
                                                :feedback="false" />
                ...
                <div class="flex justify-between space-x-4">
                    <Button v-if="activeTab > 1 && activeTab < 3" label="Previous" class="p-button-outlined w-1/3"
                        @click="activeTab--" />
                    <Button v-if="activeTab < 2" label="Next" class="p-button-outlined w-1/3 ml-auto"
                        @click="activeTab++" />
                    <Button v-if="activeTab === 2" label="Send OTP" class="p-button-outlined w-1/3"
                        :loading="isLoadingSendOtp" @click="() => sendOtp()" />
                    <Button v-if="activeTab === 3" label="Verify & Register" class="p-button-outlined w-full"
                        :loading="isLoadingVerifyOtp" @click="() => verifyOtp()" />
                </div>
                <div class="flex w-full justify-center text-center sm:text-base">
                    <span>Already have an account? <router-link :to="{ name: 'sign-in' }"
                            class="text-cinema font-medium">Sign in</router-link></span>
                </div>
            </div>
        </div>
    </VForm>
